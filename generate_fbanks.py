...
def main(tsv_file, feature_root, zip_path, output_tsv_file, n_mel_bins=80):
    ...
    with open(output_tsv_file, 'w') as f:
        f.write("\t".join(
            ['id', 'split', 'audio', 'n_frames', 'src_text', 'tgt_text', 'speaker', 'phones', 'words']) + "\n")
        for entry in dataset:
            f.write(
                "\t".join(
                    [entry['id'], entry['split'], entry['audio'], str(entry['n_frames']),
                     entry['src_text'], entry['tgt_text'], entry['speaker'], entry['phones'], entry['words']]) + "\n")


if __name__ == '__main__':
    tsv_file = ''  # Path to the input TSV file (generated by `generate_tsv.py`) containing metadata and audio paths
    feature_root = ''  # Path to a directory where individual .npy fbank feature files will be saved before zipping
    zip_root = ''  # Directory where the final ZIP archive (fbank.zip) will be created to store all .npy files
    output_tsv = ''  # Path to the output TSV file containing updated metadata, including ZIP fbanks
